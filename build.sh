...
deploy_file="/mnt/source/$project_name/deploy.snsw.$profile"
remote_deploy_file="${deploy_file##*/}"
remote_deploy_file="~/${remote_deploy_file%.*}"
...
if [[ "$action" =~ "deploy" || "$action" == "release" ]]; then
  ...
  if [ -f $deploy_file ]; then
    ...
    sshpass -p ${web_app_pw} scp $deploy_file "$web_app_un@$web_app_server:$remote_deploy_file"
    echo "OK"
  fi
    
  if [[ $profile == "local" || $profile == "dev" ]]; then
    sshpass -p ${web_app_pw} ssh $web_app_un@$web_app_server 'bash ~/deploy.snsw'
  else  
